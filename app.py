...
def init_db():
    ...
    if not c.fetchone():
        ...
        c.execute("INSERT INTO admin (usuario, senha_hash) VALUES (?, ?)", ("admin", senha_hash))
        print("Usuário admin criado com senha padrão '1234'")
...
def obter_admin():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT usuario, senha_hash FROM admin WHERE usuario = ?", ("admin",))
    admin = c.fetchone()
    conn.close()
    return admin
...
def atualizar_senha_admin(nova_senha):
    ...
    c.execute("UPDATE admin SET senha_hash = ? WHERE usuario = ?", (senha_hash, "admin"))
    conn.commit()
    conn.close()
...
def login():
    if request.method == "POST":
        ...
        admin = obter_admin()
        if admin and usuario == admin[0] and check_password_hash(admin[1], senha):
            session["logado"] = True
...
def alterar_senha():
    if request.method == "POST":
        ...
        admin = obter_admin()
        if not check_password_hash(admin[1], senha_atual):
